...
describe('describeScanSummary', () => {
  ...
  it('maps block_high to HIGH', () => {
    expect(describeScanSummary({ ...base, securityEnforcementMode: 'block_high' })).toBe(
      'Scan: ON · HIGH · 3 categories',
    );
  });

  it('maps block_medium_high to MED+HIGH', () => {
    expect(describeScanSummary({ ...base, securityEnforcementMode: 'block_medium_high' })).toBe(
      'Scan: ON · MED+HIGH · 3 categories',
    );
  });

  it('falls back to Observe when mode is null (unset)', () => {
    expect(describeScanSummary({ ...base, securityEnforcementMode: null })).toBe(
      'Scan: ON · Observe · 3 categories',
    );
  });
