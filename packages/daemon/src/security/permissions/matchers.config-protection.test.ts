 ...
 *   - Write|Edit|MultiEdit on anything under `~/.claude-sentinel/**`
 *
...
describe('matchPath: ~/.claude-sentinel/** broad protection', () => {
  it('matches settings.json under the Sentinel state dir', () => {
    expect(
      matchPath('~/.claude-sentinel/**', { file_path: `${HOME}/.claude-sentinel/settings.json` }),
    ).toBe(true);
  });

  it('matches the daemon log under the Sentinel state dir', () => {
    expect(
      matchPath('~/.claude-sentinel/**', { file_path: `${HOME}/.claude-sentinel/daemon.log` }),
    ).toBe(true);
  });

  it('matches a deeply-nested file under the Sentinel state dir', () => {
    expect(
      matchPath('~/.claude-sentinel/**', { file_path: `${HOME}/.claude-sentinel/runtime/x/y` }),
    ).toBe(true);
  });

  it('does NOT match ~/.claude-sentinelish/x (suffix-confusion guard)', () => {
    expect(
      matchPath('~/.claude-sentinel/**', { file_path: `${HOME}/.claude-sentinelish/foo.txt` }),
    ).toBe(false);
  });

  it('does NOT match ~/.claude/anything (different dir)', () => {
    expect(matchPath('~/.claude-sentinel/**', { file_path: `${HOME}/.claude/settings.json` })).toBe(
      false,
    );
  });
});
