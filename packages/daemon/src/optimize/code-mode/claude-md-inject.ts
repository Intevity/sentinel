...
function renderBlockBody(opts: CodeModeBlockOpts): string {
  ...
  return [
    ...
    '',
    '- Keep `cwd` in the body: a server configured per-project can carry',
    '  different credentials per project, and `cwd` is what selects the right',
    '  ones. Omitting it falls back to the user-scope configuration.',
    ...
    '- If a bridged server starts failing in a way that looks like the server',
    '  itself (a wedged process, a stale build, errors that began mid-session),',
    '  restart it rather than abandoning the bridge:',
    '',
    '```sh',
    `curl -s -X POST http://127.0.0.1:${opts.port}/code-mode/restart \\`,
    `  -H "Authorization: Bearer $(cat ${tokenFile})" \\`,
    `  -H 'Content-Type: application/json' \\`,
    `  -d '{"server":"<server>"}'`,
    '```',
    '',
    '  This stops the server process and everything it spawned, starts it',
    '  again, and refreshes its tool docs. An HTTP 429 means it was restarted',
    '  seconds ago — wait rather than retrying in a loop.',
